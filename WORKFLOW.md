...
- Scrie povestea în acte (3-5), fiecare act o propoziție. Pentru FIECARE trecere între
  acte alege **elementul diegetic** care maschează tăietura (lumină, cortină, rig, ușă).
  Fără punte clară = povestea nu e gata.
...
- Completează `SHOTS` în `veo-generate.mjs`: acte (travelling într-o lume) + punți
  (ultimul cadru al lumii A → primul al lumii B).
...
- Un clip = UN drum de cameră, viteză constantă. Camera care pulsează = regenerezi cu
  constrângere dură sau tai în pasul 3.
